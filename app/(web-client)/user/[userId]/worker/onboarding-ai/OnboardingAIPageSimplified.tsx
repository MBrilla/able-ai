...
// Import AI and validation utilities
import { analyzeUserIntent } from './utils/ai-systems/intent-analysis';
...
} from './utils/helpers/profile-submission';

// Import AI utilities for content generation and validation
import { generateAIVideoScript, generateAIProfileSummary, checkExistingSimilarSkill, interpretJobTitle, isUnrelatedResponse, generateContextAwarePrompt, simpleAICheck } from './utils/ai-systems/ai-utils';
import { buildJobTitleConfirmationStep, buildSimilarSkillsConfirmationStep } from './utils/step-management/step-builders';
import { setSkillName, setExperience, setWage, setAddress, setAvailability } from './utils/fieldsetters';
import { requiredFields as REQUIRED_FIELDS_CONFIG, specialFields as SPECIAL_FIELDS_CONFIG } from './utils/step-management/questions';
...
export default function OnboardingAIPageSimplified() {
  ...
  const isSpecialComponentActive = useMemo(() => {
    const currentStep = chatSteps.find(step => 
      (step.type === "calendar" || step.type === "location" || step.type === "video" || 
       ...
       step.type === "similarSkillsConfirmation" || step.type === "support" || step.type === "typing") && !step.isComplete
    );
    return !!currentStep;
  }, [chatSteps]);
  ...
  }, [formData, setFormData, chatSteps, setChatSteps, ai, workerProfileId, existingProfileData]);
  ...
  const handleSimilarSkillsUseExistingCallback = useCallback(async (fieldName: string, selectedSkill: any) => {
  ...
  const handleSimilarSkillsAddNewCallback = useCallback(async (fieldName: string, originalValue: string) => {
  ...
  const handleVideoUpload = useCallback(async (file: Blob, name?: string, stepId?: number) => {
    try {
      ...
      const fileName = `workers/${user?.uid}/introVideo/introduction-${encodeURI(user?.email ?? user?.uid ?? 'user')}.webm`;
  ...
  }, [formData, setFormData, chatSteps, setChatSteps, user, setError, existingProfileData]);
  ...
  const onSendMessage = useCallback(async (message: string) => {
    ...
    const activeInputStep = chatSteps.find((step: any) => 
      (step.type === 'input' || step.type === 'calendar' || step.type === 'location' || step.type === 'availability') && 
      !step.isComplete
    );
    ...
    if (activeInputStep && activeInputStep.inputConfig) {
      ...
      if (nextField) {
        ...
        setChatSteps((prev: any[]) => [...prev, {
          id: Date.now() + 1,
          type: "bot",
          content: "🎉 Perfect! Your worker profile is now complete. You're all set to start finding gigs!",
          isNew: true,
        }]);
      }
    }
  }, [chatSteps, formData, setFormData, setChatSteps, handleInputSubmit, getNextRequiredField, ai, workerProfileId, existingProfileData]);
