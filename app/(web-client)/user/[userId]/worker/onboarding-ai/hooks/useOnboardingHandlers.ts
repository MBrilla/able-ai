...
import {
  buildJobTitleConfirmationStep,
  buildSimilarSkillsConfirmationStep,
} from "../utils/step-management/step-builders";
import { saveSupportCaseToDatabase } from "../utils/helpers/support-utils";
import { detectEscalationTriggers } from "@/utils/escalation-detection";

/**
 * Analyze chat steps to determine if escalation is needed based on conversation patterns
 */
const analyzeChatStepsForEscalation = (
  chatSteps: any[]
  ...
  if (
    botContent.includes("I don't understand") ||
    botContent.includes("Could you clarify")
  ) {
    escalationScore += 0.4;
  ...
  if (
    userInputs.includes("help") ||
    userInputs.includes("support") ||
    userInputs.includes("human")
  ) {
    escalationScore += 0.6;
...
export function useOnboardingHandlers({
    ...
    async (
      ...
      if (typeof valueToUse === "string") {
        ...
        if (
          trimmedValue === "/manual" ||
          trimmedValue === "manual" ||
          trimmedValue.startsWith("/manual ")
        ) {
          setChatSteps((prev: any[]) => [
            ...prev,
            {
        ...
        if (
          trimmedValue === "/support" ||
          trimmedValue === "support" ||
          trimmedValue.startsWith("/support ")
        ) {
          setChatSteps((prev: any[]) => [
            ...prev,
            {
      ...
      if (chatAnalysis.shouldEscalate) {
        ...
        escalationConfidence = Math.max(
          escalationConfidence,
          chatAnalysis.confidence
        );
      }
      ...
      if (previousBotMessage && previousBotMessage.content) {
        const isUnrelated = await isUnrelatedResponse(
          valueToUse,
      ...
      if (inputName === "skills" && !formData.jobTitle) {
        ...
        const similarSkillsResult = await checkExistingSimilarSkill(
          valueToUse,
          workerProfileId || ""
        );

        if (
          similarSkillsResult.exists &&
          similarSkillsResult.similarSkills.length > 0
        ) {
          ...
          setChatSteps((prev: any[]) => [
            ...
            buildSimilarSkillsConfirmationStep(
              ...
              inputName,
              valueToUse,
              similarSkillsResult.similarSkills
            ) as unknown as any,
          ]);
        ...
        if (jobTitleResult && jobTitleResult.confidence >= 30) {
          ...
          try {
            const validationResult = await enhancedValidation(
              inputName,
              valueToUse,
              "text"
            );
            ...
            if (
              ...
              validationResult.naturalSummary
            ) {
          ...
          if (
            ...
            setChatSteps((prev: any[]) => [
              ...prev,
              {
                ...
                extractedData:
                  inputName === "equipment"
                    ? (validationResult as any).equipment
                    : validationResult.sanitized,
                isComplete: false,
                isNew: true,
              },
  ...
  const handleSanitizedConfirm = useCallback(
    async (
      ...
      try {
        setClickedSanitizedButtons(
          (prev: Set<string>) => new Set([...prev, `${fieldName}-confirm`])
        );
        ...
        if (
          ...
          const equipmentItems = String(sanitized)
            .split(", ")
  ...
  const handleJobTitleConfirm = useCallback(
    async (
      ...
      setChatSteps((prev: any[]) =>
        prev.map((step: any) =>
          step.type === "jobTitleConfirmation" && step.fieldName === fieldName
            ? { ...step, isComplete: true, confirmedChoice: "title" }
            : step
        )
      );
  ...
  const handleJobTitleReject = useCallback(
    async (fieldName: string, originalValue: string) => {
      setChatSteps((prev: any[]) =>
        prev.map((step: any) =>
          step.type === "jobTitleConfirmation" && step.fieldName === fieldName
            ? { ...step, isComplete: true, confirmedChoice: "original" }
            : step
        )
      );
  ...
  const handleSimilarSkillsAddNew = useCallback(
    async (fieldName: string, originalValue: string) => {
      setChatSteps((prev: any[]) =>
        prev.map((step: any) =>
          step.type === "similarSkillsConfirmation" &&
          step.fieldName === fieldName
            ? { ...step, isComplete: true, confirmedChoice: "new" }
            : step
        )
      );
      ...
      setChatSteps((prev: any[]) =>
        prev.map((step: any) =>
          ...
          step.confirmationConfig?.fieldName === fieldName
            ? { ...step, isComplete: true, confirmedChoice: "use-anyway" }
            : step
        )
      );
    ...
    (fieldName: string) => {
      setChatSteps((prev: any[]) =>
        prev.map((step: any) =>
          ...
          step.confirmationConfig?.type === "existing-skill-title" &&
          step.confirmationConfig?.fieldName === fieldName
            ? { ...step, isComplete: true, confirmedChoice: "change" }
            : step
        )
      );
  ...
  const handleSanitizedConfirmation = useCallback(
    async (fieldName: string, sanitizedValue: string) => {
      ...
      if (fieldName === "location") {
        ...
        const originalStep = chatSteps.find(
          (step: any) =>
            step.type === "sanitized" && step.fieldName === fieldName
        );
        if (
          originalStep &&
          originalStep.originalValue &&
          typeof originalStep.originalValue === "object"
        ) {
  ...
  const enhancedValidation = useCallback(
    async (
      field: string,
      value: unknown,
      type: string
      ...
      if (!value) {
        return {
          sufficient: false,
          clarificationPrompt:
            "Please provide some information so I can help you create the perfect worker profile!",
        };
      }
      ...
      const analysisContext: ValidationContext = {
        ...
        retryCount: unrelatedResponseCount,
        userRole: "worker",
        previousMessages: chatSteps
          .filter((step: any) => step.type === "user")
      ...
      try {
        ...
        if (field === "qualifications") {
          ...
          if (hasSkipKeywords) {
            ...
            return {
              ...
              naturalSummary:
                "Got it! You don't have any formal qualifications.",
        ...
        if (validation.isHelpRequest || validation.needsSupport) {
          const response = generateValidationResponse(
            validation,
            analysisContext
          );
          return {
        ...
        if (
          !["equipment", "hourlyRate", "wage"].includes(field) &&
          requiresEscalation(validation)
        ) {
          const escalationDetails = getEscalationDetails(
            validation,
            analysisContext,
            trimmedValue
          );
        ...
        const valueForFieldSetter =
          aiSanitizedResult?.sanitized || trimmedValue;
        ...
        switch (field) {
          ...
          default:
            ...
            const fallbackSummary = (() => {
              switch (field) {
                case "about":
                  return `Perfect! "${trimmedValue}" - that sounds great!`;
                case "skills":
                  return `Got it! You're skilled in ${trimmedValue}.`;
                case "experience":
                  return `Excellent! ${trimmedValue} experience is valuable.`;
                case "qualifications":
                  return trimmedValue.toLowerCase().includes("no formal") ||
                    trimmedValue.toLowerCase().includes("none")
                    ? `Got it! No formal qualifications needed.`
                    : `Nice! ${trimmedValue} - those are good qualifications.`;
                case "equipment":
                  return `Great! You have ${trimmedValue} ready to go.`;
                case "hourlyRate":
                  return `Perfect! £${trimmedValue} per hour is a good rate.`;
                case "location":
                  return `Awesome! ${trimmedValue} is a great location.`;
                case "availability":
                  return `Excellent! ${trimmedValue} works well.`;
                default:
                  return `Perfect! "${trimmedValue}" - that's great!`;
              }
            })();
            ...
            return {
              ...
              sanitized:
                ...
                trimmedValue,
              naturalSummary:
                aiSanitizedResult?.naturalSummary || fallbackSummary,
              extractedData:
                aiSanitizedResult?.sanitized ||
                validation.sanitizedInput ||
                trimmedValue,
        ...
        if (!validationResult.ok) {
          return {
            sufficient: false,
            clarificationPrompt:
              validationResult.error || "Please provide valid information.",
        ...
        if (validationResult.needsSupport) {
          return {
            sufficient: false,
            clarificationPrompt:
              validationResult.supportMessage || "Support assistance needed.",
            sanitized: validation.sanitizedInput || trimmedValue,
          };
        }
        ...
        if (
          field === "equipment" &&
          (validationResult as any).equipment &&
          (validationResult as any).equipment.length > 0
        ) {
          // Convert equipment array to string for display
          sanitizedValue = (validationResult as any).equipment
          ...
          sanitizedValue =
            location.formatted_address || `${location.lat}, ${location.lng}`;
          extractedData = location; // Store the full location object
        } else if (
          field === "experience" &&
          (validationResult as any).experienceText
        ) {
          ...
          sanitizedValue =
            ...
            (validationResult.wage
              ? String(validationResult.wage.amount)
              : null) ||
        ...
        const fallbackSummary = (() => {
          ...
          switch (field) {
            case "about":
              return `Perfect! "${displayValue}" - that sounds great!`;
            case "skills":
              return `Got it! You're a ${displayValue}.`;
            case "experience":
              return `Excellent! ${displayValue} level experience is valuable.`;
            case "qualifications":
              return displayValue.toLowerCase().includes("no formal") ||
                displayValue.toLowerCase().includes("none")
                ? `Got it! No formal qualifications needed.`
                : `Nice! ${displayValue} - those are great qualifications.`;
            case "equipment":
              return `Great! You have ${displayValue} ready to go.`;
            case "hourlyRate":
              return `Perfect! £${displayValue} per hour is a good rate.`;
            case "location":
              return `Awesome! ${displayValue} is a great location.`;
            case "availability":
              return `Excellent! ${displayValue} works well.`;
            default:
              return `Perfect! "${displayValue}" - that's great!`;
          }
        })();
  ...
  const handleManualFormSubmit = useCallback(
    async (formData: any) => {
      try {
        ...
        const { saveWorkerProfileFromOnboardingAction } = await import(
          "@/actions/user/gig-worker-profile"
        );
  ...
  const hasActiveStepForField = useCallback(
    (fieldName: string) => {
      return chatSteps.some(
        (step: any) =>
          (step.type === "input" ||
            step.type === "sanitized" ||
            step.type === "confirmation" ||
  ...
  const getNextRequiredField = useCallback(
    (formData: any) => {
      ...
      const hasLocationAndAvailability = !!(
        formData.location && formData.availability
      );
  ...
  const isActiveInputStep = useCallback(
    (step: any, idx: number) => {
  ...
  const handleProfileSubmission = useCallback(
    async (summaryData: any) => {
      ...
      try {
        ...
        const experienceText =
          summaryData?.experience || formData.experience || "";

        const { years: experienceYears, months: experienceMonths } =
          parseExperienceToNumeric(experienceText);
        ...
        const requiredData = {
          ...
          qualifications: (() => {
            const quals =
              summaryData?.qualifications || formData.qualifications || "";
            if (
              typeof quals === "string" &&
              (quals.toLowerCase().includes("no formal") ||
                quals.toLowerCase().includes("none"))
            ) {
              return null;
            }
            ...
            const cleanedQuals =
              typeof quals === "string"
                ? quals.replace(/^Qualifications\s*:\s*/i, "").trim()
                : quals;
            return cleanedQuals;
          })(),
          equipment: Array.isArray(summaryData?.equipment)
            ...
            ? formData.equipment
            : typeof summaryData?.equipment === "string" &&
              summaryData.equipment.trim().length > 0
            ? summaryData.equipment
                .split(/[,\n;]/)
                .map((item: string) => ({
                  name: item.trim(),
          ...
          hourlyRate: (() => {
            const rateValue =
              summaryData?.hourlyRate || formData.hourlyRate || "";
